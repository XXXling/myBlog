...
public class BackCommentController {
    ...
    public String commentListView(@RequestParam(required = false, defaultValue = "1") Integer pageIndex,
                                  ...
                                  Model model) {
        PageInfo<BlogComment> commentPageInfo = commentService.listCommentByPage(pageIndex, pageSize);
    ...
    @ResponseBody
    public void insertComment(HttpServletRequest request, BlogComment comment) {
        //添加评论
        comment.setCommentIp(MyUtils.getIpAddr(request));
        comment.setCommentCreateTime(new Date());
        commentService.insertComment(comment);
        //更新文章的评论数
        Article article = articleService.getArticleDetails(null, comment.getCommentArticleId());
    ...
    public void deleteComment(@PathVariable("id") Integer id) {
        BlogComment comment = commentService.getCommentById(id);

        //删除评论及其子评论
        List<BlogComment> childCommentList = commentService.listChildComment(id);
        if (childCommentList != null) {
            childCommentList.add(comment);
        }
        commentService.deleteComments(childCommentList);
    ...
    public String editCommentSubmit(BlogComment comment) {
    ...
    public String replyCommentSubmit(HttpServletRequest request, BlogComment comment) {
        //文章评论数+1
