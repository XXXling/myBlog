...
import com.huining.ssm.blog.dto.ArticleParam;
...
public class BackArticleController {
    ...
    public String insertArticleSubmit(HttpSession session, ArticleParam articleParam) {
        ...
        if (articleParam.getArticleChildCategoryId() != null) {
            categoryList.add(new BlogCategory(articleParam.getArticleParentCategoryId()));
        }
        if (articleParam.getArticleChildCategoryId() != null) {
            categoryList.add(new BlogCategory(articleParam.getArticleChildCategoryId()));
        }
        ...
        if (articleParam.getArticleTagIds() != null) {
            for (int i = 0; i < articleParam.getArticleTagIds().size(); i++) {
                BlogTag tag = new BlogTag(articleParam.getArticleTagIds().get(i));
                tagList.add(tag);
            }
        }
    ...
    public ModelAndView editArticleView(@PathVariable("id") Integer id) {
        ...
        Article article = articleService.getArticleDetails(null, id);
